...
cd "%~dp0"
if not exist "%pkgsource%\%%A\apps-temp\XMB Manager Plus\XMB Manager Plus\%%X\%%O\dev_flash~vsh~resource" mkdir "%pkgsource%\%%A\apps-temp\XMB Manager Plus\XMB Manager Plus\%%X\%%O\dev_flash~vsh~resource"
move "%pkgsource%\%%A\%id_xmbmp%\USRDIR\apps\XMB Manager Plus\%%X\%%O\dev_flash~vsh~resource\%%C" "%pkgsource%\%%A\apps-temp\XMB Manager Plus\XMB Manager Plus\%%X\%%O\dev_flash~vsh~resource\" >NUL
)
)
)
echo - Compiling elf ...
...
FOR /f "tokens=1,2 delims=*" %%C IN ('dir /b "%pkgsource%\%%A\apps-temp\XMB Manager Plus\XMB Manager Plus\%%X\%%O\dev_flash~vsh~resource\*."') DO (
del /Q "%pkgsource%\%%A\%id_xmbmp%\USRDIR\apps\XMB Manager Plus\%%X\%%O\dev_flash~vsh~resource\*.rco" >NUL
move "%pkgsource%\%%A\apps-temp\XMB Manager Plus\XMB Manager Plus\%%X\%%O\dev_flash~vsh~resource\%%C" "%pkgsource%\%%A\%id_xmbmp%\USRDIR\apps\XMB Manager Plus\%%X\%%O\dev_flash~vsh~resource\" >NUL
)
)
)
